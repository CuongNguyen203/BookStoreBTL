...
const AdminBook = () => {
    ...
    const category = ["Lãng Mạn", "Kinh Dị", "Trinh Thám", "Khoa Học", "Kinh Điển", "Chiến Tranh", "Kì Bí"];
    ...
    return (
        <div>
            ...
            <form className="form book-info">
                <div style={{ width: "100%" }}>
                    <div className="one-line">
                        <div className=" book-text ">
                            ...
                            <input required disabled={formDisabled} type="text" id='title' name="title" className="form-control " onChange={onUpdate} defaultValue={book.title} />
                ...
                <div>
                    <label className="btn btn-outline-danger" htmlFor="formFile">Upload</label>
            ...
            <div className="footer">
                <br />
                {adminOption === "Edit" && <button className="btn btn-outline-primary mx-2" onClick={() => { setAdminOption("Save"); setFormDisabled(false) }}>{adminOption}</button>}
                {adminOption === "Save" && <button className="btn btn-outline-primary mx-2" onClick={onSaveClick}>{adminOption}</button>}
                {adminOption === "Add" && <button className="btn btn-outline-primary mx-2" onClick={onAddClick}>{adminOption}</button>}
            </div>
        </div>
    );
}
